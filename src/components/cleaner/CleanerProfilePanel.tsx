...
interface VettingRecord { address_line_1:string|null;address_line_2:string|null;city:string|null;citizenship_route:string|null;identity_status:string;address_status:string;right_to_work_status:string;right_to_work_basis:string|null;right_to_work_expires_on:string|null;dbs_status:string;submitted_at:string|null; }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [capabilityResult, documentResult, vettingResult] = await Promise.all([
        ...
        db.from("cleaner_vetting_records").select("address_line_1,address_line_2,city,citizenship_route,identity_status,address_status,right_to_work_status,right_to_work_basis,right_to_work_expires_on,dbs_status,submitted_at").eq("cleaner_id",profile.id).maybeSingle(),
      ]);
      ...
      if (!cancelled) {
        ...
        setVetting((vettingResult.data as VettingRecord | null) || null);
        setLoading(false);
      }
    };
    load();
