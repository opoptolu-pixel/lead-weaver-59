  ...
  const fetchDashboard = useCallback(async () => {
    ...
    if (profileData) {
      ...
      if (initialAssignmentResult.error?.message.includes("quality_review_status")) {
        const legacyResult = await db.from("job_assignments").select(`id,status,offered_at,job:jobs(id,reference,status,general_location,scheduled_date,start_time,expected_duration_minutes,cleaner_payout_pence,requirements,service_type:service_types(name),customer:customers(name,phone),address:customer_addresses(address_line_1,address_line_2,city,postcode,access_notes),accepted_quote:quotes!jobs_accepted_quote_id_fkey(add_ons:quote_addons(id,addon_name,quantity,unit_cleaner_payout_pence,unit_duration_minutes)))`).eq("cleaner_id", profileData.id).order("offered_at", { ascending: false });
        assignmentResult = {
          ...legacyResult,
