...
function JobList({ assignments, evidence, busy, completionNotes, setCompletionNotes, onRespond, onUpload, onComplete }: { assignments: Assignment[]; evidence: Evidence[]; busy: string | null; completionNotes: Record<string,string>; setCompletionNotes: React.Dispatch<React.SetStateAction<Record<string,string>>>; onRespond: (id:string,response:"accepted"|"declined")=>void; onUpload:(a:Assignment,t:"before"|"after",f:FileList|null)=>void; onComplete:(a:Assignment)=>void; }) {
  ...
  return <div className="grid gap-5">{assignments.map((assignment) => {
    const photos = evidence.filter((item) => item.assignment_id === assignment.id); const before = photos.filter((p) => p.evidence_type === "before"); const after = photos.filter((p) => p.evidence_type === "after"); const revealsDetails = ["accepted","completed"].includes(assignment.status);
