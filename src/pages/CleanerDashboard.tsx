...
interface Payout { id: string; job_id: string; amount_pence: number; status: string; held_reason: string | null; paid_at: string | null; job: { reference: string; scheduled_date: string; status: string; quality_review_status: string; service_type: { name: string } }; }
...
const cleanerPayoutLabel = (payout: Payout) => payout.status === "paid" ? "Paid" : payout.status === "approved" ? "Approved for Friday pay run" : payout.status === "processing" ? "Payment processing" : payout.job.status === "quality_check" ? "Awaiting Cleanda quality review" : payout.job.status === "issue" ? "Held — job issue" : payout.held_reason?.toLowerCase().includes("bank") ? "Held — bank verification needed" : ["assigned","in_progress","offered","awaiting_assignment"].includes(payout.job.status) ? "Work not yet approved" : payout.status.replace(/_/g," ");
...
export default function CleanerDashboard() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    if (profileData) {
      const [initialAssignmentResult, evidenceResult, payoutResult, availabilityResult, timeResult, checklistResult] = await Promise.all([
        db.from("job_assignments").select(`id,status,offered_at,job:jobs(id,reference,status,recurring_plan_id,recurring_visit_id,general_location,scheduled_date,start_time,expected_duration_minutes,cleaner_payout_pence,requirements,quality_review_status,quality_review_notes,cleaner_completion_notes,service_type:service_types(name),customer:customers(name,phone),address:customer_addresses(address_line_1,address_line_2,city,postcode,access_notes),accepted_quote:quotes!jobs_accepted_quote_id_fkey(add_ons:quote_addons(id,addon_name,quantity,unit_cleaner_payout_pence,unit_duration_minutes)))`).eq("cleaner_id", profileData.id).order("offered_at", { ascending: false }),
        ...
        db.from("cleaner_payouts").select("id,job_id,amount_pence,status,held_reason,paid_at,job:jobs(reference,scheduled_date,status,quality_review_status,service_type:service_types(name))").eq("cleaner_id", profileData.id).order("created_at", { ascending: false }),
      ...
      if (initialAssignmentResult.error?.message.includes("quality_review_status")) {
        const legacyResult = await db.from("job_assignments").select(`id,status,offered_at,job:jobs(id,reference,status,recurring_plan_id,recurring_visit_id,general_location,scheduled_date,start_time,expected_duration_minutes,cleaner_payout_pence,requirements,service_type:service_types(name),customer:customers(name,phone),address:customer_addresses(address_line_1,address_line_2,city,postcode,access_notes),accepted_quote:quotes!jobs_accepted_quote_id_fkey(add_ons:quote_addons(id,addon_name,quantity,unit_cleaner_payout_pence,unit_duration_minutes)))`).eq("cleaner_id", profileData.id).order("offered_at", { ascending: false });
        assignmentResult = {
          ...legacyResult,
  ...
  return <div className="cleaner-dashboard">
    ...
    <main className="mx-auto max-w-6xl space-y-8 px-4 sm:px-6">
      <div className="flex flex-col gap-4 border-b border-border/70 pb-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.12em] text-secondary">Cleaner workspace</p><h1 className="mt-2 text-3xl font-bold tracking-[-0.045em] sm:text-4xl">Welcome, {profile.full_name || "Cleaner"}</h1></div><div className="flex flex-wrap gap-2"><Badge variant="outline">Application: {profile.application_status}</Badge><Badge variant="outline">Verification: {profile.verification_status.replace(/_/g, " ")}</Badge><Badge variant="outline">Status: {profile.operational_status}</Badge></div></div>
      ...
      <Tabs value={activeTab} onValueChange={changeTab} className="space-y-6">
        <nav aria-label="Cleaner dashboard sections" className="overflow-x-auto rounded-2xl border border-border/70 bg-card p-2 shadow-sm">
        ...
        <TabsContent value="profile"><CleanerProfilePanel profile={profile} email={user?.email} userId={user?.id} /></TabsContent>
...
function JobList({ assignments, evidence, timeEntries, checklist, busy, completionNotes, setCompletionNotes, onRespond, onUpload, onComplete, onClock, onToggleChecklist }: { assignments: Assignment[]; evidence: Evidence[]; timeEntries:TimeEntry[]; checklist:ChecklistItem[]; busy: string | null; completionNotes: Record<string,string>; setCompletionNotes: React.Dispatch<React.SetStateAction<Record<string,string>>>; onRespond: (id:string,response:"accepted"|"declined")=>void; onUpload:(a:Assignment,t:"before"|"after",f:FileList|null)=>void; onComplete:(a:Assignment)=>void; onClock:(id:string,a:"in"|"out")=>void; onToggleChecklist:(i:ChecklistItem,c:boolean)=>void; }) {
  ...
  return <div className="grid gap-5">{assignments.map((assignment) => {
    ...
    return <article key={assignment.id} className="rounded-xl border bg-card p-5 shadow-sm"><div className="flex flex-col gap-5 lg:flex-row lg:justify-between"><div className="space-y-3"><div className="flex flex-wrap items-center gap-2"><strong>{assignment.job.reference}</strong><Badge>{assignment.status}</Badge>{assignment.job.recurring_plan_id && <Badge variant="secondary">Recurring visit</Badge>}<span>{assignment.job.service_type.name}</span></div><div className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2"><span><Calendar className="mr-2 inline h-4 w-4" />{assignment.job.scheduled_date}</span><span><Clock className="mr-2 inline h-4 w-4" />{assignment.job.start_time?.slice(0,5) || "Time awaiting confirmation"}{assignment.job.expected_duration_minutes ? ` · ${assignment.job.expected_duration_minutes / 60} hrs` : ""}</span><span><MapPin className="mr-2 inline h-4 w-4" />{revealsDetails ? address : `${assignment.job.general_location}, Greater Manchester`}</span><span><PoundSterling className="mr-2 inline h-4 w-4" />Cleaner payout: {money(assignment.job.cleaner_payout_pence)}</span></div>{revealsDetails && assignment.job.customer && <p className="rounded-lg bg-muted/50 p-3 text-sm"><strong>Customer:</strong> {assignment.job.customer.name} · {assignment.job.customer.phone}{assignment.job.address?.access_notes && <><br /><strong>Access:</strong> {assignment.job.address.access_notes}</>}</p>}{!!assignment.job.accepted_quote?.add_ons?.length && <div className="rounded-lg border bg-emerald-50/50 p-3 text-sm"><strong>Service add-ons</strong><p className="mt-1 text-xs text-muted-foreground">Included in this job and in your total payout above.</p><ul className="mt-2 space-y-1">{assignment.job.accepted_quote.add_ons.map((addOn) => <li key={addOn.id} className="flex flex-wrap justify-between gap-2"><span>{addOn.addon_name}{addOn.quantity > 1 ? ` × ${addOn.quantity}` : ""}</span><span className="text-muted-foreground">+{(addOn.unit_duration_minutes * addOn.quantity) / 60} hrs · {money(addOn.unit_cleaner_payout_pence * addOn.quantity)}</span></li>)}</ul></div>}{visibleRequirements && <p className="whitespace-pre-wrap text-sm"><strong>Job requirements:</strong> {visibleRequirements}</p>}</div><div className="flex shrink-0 gap-2">{assignment.status === "offered" && <><Button onClick={() => onRespond(assignment.id,"accepted")} disabled={busy === assignment.id}>Accept</Button><Button variant="outline" onClick={() => onRespond(assignment.id,"declined")} disabled={busy === assignment.id}>Decline</Button></>}</div></div>
