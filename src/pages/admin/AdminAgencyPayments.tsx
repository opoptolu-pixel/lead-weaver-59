...
interface Payout {
  ...
  paid_at: string | null;
  cleaner: { full_name: string | null; bank_details_status: string; bank_sort_code_last2: string | null; bank_account_last4: string | null };
  job: { reference: string; scheduled_date: string; status: string; customer_amount_pence: number; service_type: { name: string } };
}
...
export default function AdminAgencyPayments() {
  ...
  const fetchData = async () => {
    ...
    const [collectionResult, payoutResult] = await Promise.all([
      ...
      db.from("cleaner_payouts").select("id,amount_pence,status,earning_week_start,scheduled_pay_date,held_reason,bank_transfer_reference,paid_at,cleaner:cleaner_profiles(full_name,bank_details_status,bank_sort_code_last2,bank_account_last4),job:jobs(reference,scheduled_date,status,customer_amount_pence,service_type:service_types(name))").order("scheduled_pay_date", { ascending: false }),
    ]);
    setLoading(false);
