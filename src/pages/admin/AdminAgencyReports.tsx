...
import { AlertTriangle, BriefcaseBusiness, CheckCircle2, Download, Loader2, PoundSterling, RefreshCw, Search, TrendingUp, UsersRound, WalletCards, XCircle } from "lucide-react";
...
interface ReportData {
  ...
  jobs: ReportJob[];
  payments: Array<{ id: string; amount_pence: number; status: string; paid_at: string | null; created_at: string; provider: string | null; provider_reference: string | null; job: { id: string; reference: string; customer: { name: string } | null; service_type: { name: string } | null } | null }>;
...
export default function AdminAgencyReports() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    const [requests, jobs, payments, payouts, issues] = await Promise.all([
      ...
      supabase.from("customer_payments").select("id,amount_pence,status,paid_at,created_at,provider,provider_reference,job:jobs(id,reference,customer:customers(name),service_type:service_types(name))"),
  ...
  const matches = (...values: Array<string | null | undefined>) => !query || values.filter(Boolean).join(" ").toLowerCase().includes(query);
  const visibleJobs = report.jobs.filter((job) => matches(job.reference, job.customer?.name, job.service_type?.name, job.address?.postcode, job.status));
  const visiblePayments = report.payments.filter((payment) => matches(payment.job?.reference, payment.job?.customer?.name, payment.job?.service_type?.name, payment.provider, payment.status));
  const visiblePayouts = data.payouts.filter((payout) => matches(payout.job?.reference, payout.cleaner?.full_name, payout.status, payout.scheduled_pay_date));
  const statusCounts = report.jobs.reduce<Record<string, number>>((counts, job) => { counts[job.status] = (counts[job.status] || 0) + 1; return counts; }, {});
  const { start: periodStart, end: periodEnd } = getDateFilter();
  const periodLabel = `${periodStart.toLocaleDateString("en-GB", { dateStyle: "medium" })} – ${periodEnd.toLocaleDateString("en-GB", { dateStyle: "medium" })}`;

  return <AdminLayout title="Reports"><div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">Agency reports</h1><p className="text-muted-foreground">Commercial, fulfilment and cleaner performance for the selected period.</p><Badge variant="outline" className="mt-2">Reporting period: {periodLabel}</Badge></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={exportOperations} disabled={!report.jobs.length}><Download className="mr-2 h-4 w-4" />Operations CSV</Button><Button variant="outline" onClick={exportFinance} disabled={!data.payouts.length}><Download className="mr-2 h-4 w-4" />Payouts CSV</Button><Button variant="outline" onClick={() => void fetchReports()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</Button></div></div>
    {loading ? <div className="flex min-h-72 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin" /></div> : <>
      ...
      <Card><CardHeader><CardTitle>Job pipeline breakdown</CardTitle><CardDescription>Every job in the selected period grouped by its current operational stage.</CardDescription></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6">{Object.entries(statusCounts).sort((a,b)=>b[1]-a[1]).map(([status,count]) => <div key={status} className="rounded-lg border bg-muted/20 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{status.split("_").join(" ")}</p><p className="mt-2 text-2xl font-bold">{count}</p></div>)}</div></CardContent></Card>

      <Card><CardHeader><CardTitle>Report drill-down</CardTitle><CardDescription>Trace headline figures to the individual booking, payment and payout records behind them.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search reference, customer, cleaner, service, postcode or status" className="pl-9" /></div><Tabs defaultValue="bookings"><TabsList className="flex h-auto flex-wrap justify-start"><TabsTrigger value="bookings">Bookings ({visibleJobs.length})</TabsTrigger><TabsTrigger value="payments">Customer payments ({visiblePayments.length})</TabsTrigger><TabsTrigger value="payouts">Cleaner payouts ({visiblePayouts.length})</TabsTrigger></TabsList>
        <TabsContent value="bookings" className="mt-4"><AuditTable empty="No booking records match this search."><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Reference</TableHead><TableHead>Customer / service</TableHead><TableHead>Postcode</TableHead><TableHead>Status</TableHead><TableHead>Customer</TableHead><TableHead>Cleaner</TableHead><TableHead>Margin</TableHead><TableHead>Quality</TableHead></TableRow></TableHeader><TableBody>{visibleJobs.map((job) => <TableRow key={job.id}><TableCell className="whitespace-nowrap">{job.scheduled_date}</TableCell><TableCell className="font-medium">{job.reference}</TableCell><TableCell><p>{job.customer?.name || "—"}</p><p className="text-xs text-muted-foreground">{job.service_type?.name || "—"}</p></TableCell><TableCell>{job.address?.postcode || "—"}</TableCell><TableCell><Badge variant="outline">{job.status.split("_").join(" ")}</Badge></TableCell><TableCell>{money(job.customer_amount_pence)}</TableCell><TableCell>{money(job.cleaner_payout_pence)}</TableCell><TableCell className={(job.customer_amount_pence-job.cleaner_payout_pence)<0 ? "text-red-600" : "text-emerald-700"}>{money(job.customer_amount_pence-job.cleaner_payout_pence)}</TableCell><TableCell>{job.quality_review_status.split("_").join(" ")}</TableCell></TableRow>)}</TableBody></AuditTable></TabsContent>
        <TabsContent value="payments" className="mt-4"><AuditTable empty="No customer payments match this search."><TableHeader><TableRow><TableHead>Paid at</TableHead><TableHead>Job</TableHead><TableHead>Customer</TableHead><TableHead>Service</TableHead><TableHead>Amount</TableHead><TableHead>Provider</TableHead><TableHead>Status</TableHead><TableHead>Reference</TableHead></TableRow></TableHeader><TableBody>{visiblePayments.map((payment) => <TableRow key={payment.id}><TableCell className="whitespace-nowrap">{payment.paid_at ? new Date(payment.paid_at).toLocaleString("en-GB") : "Not paid"}</TableCell><TableCell className="font-medium">{payment.job?.reference || "—"}</TableCell><TableCell>{payment.job?.customer?.name || "—"}</TableCell><TableCell>{payment.job?.service_type?.name || "—"}</TableCell><TableCell>{money(payment.amount_pence)}</TableCell><TableCell>{payment.provider || "offline"}</TableCell><TableCell><Badge variant="outline">{payment.status}</Badge></TableCell><TableCell className="max-w-40 truncate" title={payment.provider_reference || ""}>{payment.provider_reference || "—"}</TableCell></TableRow>)}</TableBody></AuditTable></TabsContent>
        <TabsContent value="payouts" className="mt-4"><AuditTable empty="No cleaner payouts match this search."><TableHeader><TableRow><TableHead>Pay date</TableHead><TableHead>Job</TableHead><TableHead>Cleaner</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead><TableHead>Paid at</TableHead></TableRow></TableHeader><TableBody>{visiblePayouts.map((payout) => <TableRow key={payout.id}><TableCell>{payout.scheduled_pay_date || "Not scheduled"}</TableCell><TableCell className="font-medium">{payout.job?.reference || "—"}</TableCell><TableCell>{payout.cleaner?.full_name || "—"}</TableCell><TableCell>{money(payout.amount_pence)}</TableCell><TableCell><Badge variant="outline">{payout.status.split("_").join(" ")}</Badge></TableCell><TableCell>{payout.paid_at ? new Date(payout.paid_at).toLocaleString("en-GB") : "—"}</TableCell></TableRow>)}</TableBody></AuditTable></TabsContent>
...
function AuditTable({ children, empty }: { children: React.ReactNode; empty: string }) { const body = Array.isArray(children) ? children[1] : null; const hasRows = Boolean(body && typeof body === "object" && "props" in body && Array.isArray(body.props.children) && body.props.children.length); return hasRows ? <div className="overflow-x-auto rounded-lg border"><Table>{children}</Table></div> : <div className="rounded-lg border border-dashed p-10 text-center text-muted-foreground">{empty}</div>; }
