...
async function sendSMSMessage(to: string, body: string) {
  ...
  if (!from) {
    throw new Error("Missing Twilio SMS number (TWILIO_SMS_FROM)");
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { leadId, autoPublishHours = 24 }: ConfirmationRequest = await req.json();
    logStep("Received confirmation request", { leadId, autoPublishHours });
