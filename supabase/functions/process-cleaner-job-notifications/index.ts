...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    for(const item of due||[]){
      ...
      if(!claimed)continue;
      try{
        const {data:a,error:aErr}=await db.from("job_assignments").select("id,status,cleaner:cleaner_profiles(user_id,full_name,phone),job:jobs(reference,status,scheduled_date,start_time,expected_duration_minutes,cleaner_payout_pence,requirements,service_request:service_requests(source),service_type:service_types(name),customer:customers(name,email),address:customer_addresses(address_line_1,address_line_2,city,postcode,access_notes))").eq("id",item.assignment_id).single();
        if(aErr||!a)throw aErr||new Error("Assignment not found");
        const j=a.job as any;
        if(j.service_request?.source==="e2e_test_seed"){await db.from("cleaner_job_notifications").update({status:"cancelled",last_error:"Test job notifications are disabled",updated_at:new Date().toISOString()}).eq("id",item.id);results.push({id:item.id,status:"cancelled"});continue;}
        if(item.notification_type!=="offer_email"&&!['assigned','in_progress'].includes(j.status)){await db.from("cleaner_job_notifications").update({status:"cancelled",last_error:`Job is ${j.status}`,updated_at:new Date().toISOString()}).eq("id",item.id);results.push({id:item.id,status:"cancelled"});continue;}
        ...
        if(item.notification_type==="offer_email"&&(a.status!=="offered"||j.status!=="offered"))throw new Error("Offer is no longer active");
