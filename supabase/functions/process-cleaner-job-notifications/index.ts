...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    for(const item of due||[]){
      ...
      if(!claimed)continue;
      try{
        const {data:a,error:aErr}=await db.from("job_assignments").select("id,status,cleaner:cleaner_profiles(user_id,full_name,phone),job:jobs(reference,status,scheduled_date,start_time,expected_duration_minutes,cleaner_payout_pence,requirements,service_type:service_types(name),customer:customers(name),address:customer_addresses(address_line_1,address_line_2,city,postcode,access_notes))").eq("id",item.assignment_id).single();
        ...
        const exact=["assigned_email","reminder_3_day_email","reminder_1_day_email","reminder_day_email","reminder_day_sms"].includes(item.notification_type);
        ...
        const label:any={offer_email:"New Cleanda job offer",assigned_email:"Your Cleanda job is confirmed",reminder_3_day_email:"Reminder: cleaning job in 3 days",reminder_1_day_email:"Reminder: cleaning job tomorrow",reminder_day_email:"Reminder: cleaning job today",reminder_day_sms:"Cleanda job reminder"};
        const details=`${j.service_type.name} on ${when} at ${j.start_time?.slice(0,5)||"time TBC"}. ${j.expected_duration_minutes?`${j.expected_duration_minutes/60} hours. `:""}Location: ${address}. Payout after approved completion: ${money(j.cleaner_payout_pence)}. Ref: ${j.reference}.`;
        let providerRef="";
        if(item.channel==="email"){
          const {data,error:e}=await resend.emails.send({from:"Cleanda <hello@cleanda.co.uk>",to:[user.user.email],subject:label[item.notification_type],html:`<div style="font-family:Arial;max-width:620px;margin:auto"><h2>${esc(label[item.notification_type])}</h2><p>Hi ${esc(cleaner.full_name||"there")},</p><p>${esc(details)}</p>${exact?`<p><strong>Customer:</strong> ${esc(j.customer.name)}</p><p><strong>Access notes:</strong> ${esc(j.address.access_notes||"None")}</p><p><strong>Instructions:</strong> ${esc(j.requirements||"Standard Cleanda checklist applies")}</p>`:"<p>Open your Cleanda dashboard to accept or decline this offer. The full address is released after acceptance.</p>"}<p>Cleanda Operations</p></div>`});if(e)throw e;providerRef=data?.id||"";
        }else{
