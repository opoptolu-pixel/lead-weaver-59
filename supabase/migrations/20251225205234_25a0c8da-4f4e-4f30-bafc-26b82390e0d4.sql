...
-- Recreate with property_type, bedrooms, and frequency
CREATE OR REPLACE FUNCTION public.get_user_leads_with_access_control(p_user_id uuid)
 RETURNS TABLE(id uuid, postcode text, job_type text, display_value text, date date, unlocked_at timestamp with time zone, job_status text, job_notes text, job_completed_at timestamp with time zone, value integer, customer_name text, customer_email text, customer_phone text, customer_address text, access_expires_at timestamp with time zone, is_access_expired boolean, property_type text, bedrooms text, frequency text)
 LANGUAGE plpgsql
