...
-- Update deduct_credit_atomic to use granted credits first and set credit_type
...
BEGIN
  ...
  SELECT credits, granted_credits, is_suspended, suspension_reason, is_verified, leads_purchased
  INTO v_credits, v_granted_credits, v_is_suspended, v_suspension_reason, v_is_verified, v_leads_purchased
  FROM profiles
  ...
  -- Granted credits are used first
  IF v_granted_credits > 0 THEN
    v_credit_type := 'granted';
    -- Deduct from granted_credits
    UPDATE profiles
    SET 
      credits = credits - 1,
