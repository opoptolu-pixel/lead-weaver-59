-- Guarantee that every accepted/completed cleaner assignment has one payout
-- ledger row. This repairs historical gaps and prevents future ones even when
-- an assignment transition is performed outside respond_to_job_assignment().
...
-- Repair missing ledgers without modifying any existing payout.
INSERT INTO public.cleaner_payouts (
  job_id, cleaner_id, amount_pence, currency, status, held_reason, approved_at
)
SELECT
