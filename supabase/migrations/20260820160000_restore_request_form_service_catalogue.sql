-- Restore the specialist request-form services that pre-date the managed-agency
-- pivot. Each remains a manually quoted Cleanda service, not a marketplace lead.
...
-- Upgrade the default selection created by the prior migration. A selection that
-- an administrator has already customised is deliberately left untouched.
